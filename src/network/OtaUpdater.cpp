...
OtaUpdater::OtaUpdaterError OtaUpdater::checkForUpdate() {
  ...
  for (int attempt = 1; attempt <= otaHttpMaxAttempts; ++attempt) {
    ReleaseNotesScanner notesScanner;
    ...
    const bool ok = HttpDownloader::fetchUrlVerified(
        releaseApiUrl,
        [&releaseParser, &notesScanner, &bytesSeen](const uint8_t* data, size_t len) {
          bytesSeen += len;
          if (bytesSeen > releaseMetadataMaxBytes) {
            return false;
          }
          releaseParser.feed(reinterpret_cast<const char*>(data), len);
          notesScanner.feed(reinterpret_cast<const char*>(data), len);
          // Stop early once everything the dialog needs is in hand. "body" sits after the assets
          // in GitHub's response, so waiting for it costs the tail of a small JSON document; a
          // release with no notes simply runs to the end of the stream.
          if (releaseParser.foundTag() && releaseParser.foundFirmware() && notesScanner.done()) {
            return false;
          }
          return true;
        },
    ...
    releaseNotes = notesScanner.text();
